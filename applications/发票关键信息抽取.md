...
- [基于VI-LayoutXLM的发票关键信息抽取](#基于vi-layoutxlm的发票关键信息抽取)
  - [1. 项目背景及意义](#1-项目背景及意义)
  - [2. 项目内容](#2-项目内容)
  - [3. 安装环境](#3-安装环境)
  - [4. 关键信息抽取](#4-关键信息抽取)
    - [4.1 文本检测](#41-文本检测)
    - [4.2 文本识别](#42-文本识别)
    - [4.3 语义实体识别 （Semantic Entity Recognition）](#43-语义实体识别-semantic-entity-recognition)
      - [4.3.1 准备数据](#431-准备数据)
      - [4.3.2 开始训练](#432-开始训练)
      - [4.3.3 模型评估](#433-模型评估)
      - [4.3.4 模型预测](#434-模型预测)
    - [4.4 关系抽取（Relation Extraction）](#44-关系抽取relation-extraction)
      - [4.4.1 准备数据](#441-准备数据)
      - [4.4.2 开始训练](#442-开始训练)
      - [4.4.3 模型评估](#443-模型评估)
      - [4.4.4 模型预测](#444-模型预测)
...
# SER单卡训练
python3 tools/train.py -c configs/kie/vi_layoutxlm/ser_vi_layoutxlm_xfund_zh_udml.yml

# SER多卡训练，通过--gpus参数指定卡号
python3 -m paddle.distributed.launch --gpus '0,1,2,3'  tools/train.py -c configs/kie/vi_layoutxlm/ser_vi_layoutxlm_xfund_zh_udml.yml
```
...
```bash
python3 tools/infer_kie_token_ser.py -c fapiao/ser_vi_layoutxlm.yml \
  -o Architecture.Backbone.checkpoints=fapiao/models/ser_vi_layoutxlm_fapiao_udml/best_accuracy \
